...
class ApiController extends BaseController
{
    ...
    public function seedDatabase()
    {
        ...
        $db->query("SET FOREIGN_KEY_CHECKS = 0;");
        $db->query("TRUNCATE TABLE PAGOS;");
        $db->query("TRUNCATE TABLE FACTURAS;");
        $db->query("TRUNCATE TABLE BITACORA_SORTEO;");
        $db->query("TRUNCATE TABLE COTIZACIONES;");
        $db->query("TRUNCATE TABLE CAT_CLIENTES;");
        $db->query("SET FOREIGN_KEY_CHECKS = 1;");
        ...
        foreach ($cotizaciones as $c) {
            $db->table('COTIZACIONES')->insert([
                ...
                'ID_Cliente' => $c[1],
                'PO_Referencia' => $c[2],
                'Monto_Autorizado' => $c[3],
                'Piezas_Autorizadas' => $c[4],
                'Estatus' => $c[5]
            ]);
        }
        ...
        foreach ($sorteos as $s) {
            $db->table('BITACORA_SORTEO')->insert([
                'ID_Captura' => $s[0],
                'Fecha' => $s[1],
                'ID_Cotizacion' => $s[2],
                'Horas_Trabajadas' => $s[3],
                'Piezas_Sorteadas' => $s[4],
                'Monto_Devengado' => $s[5],
                'Estatus_Facturacion' => $s[6]
            ]);
        }
